...
fn gen_named(
    name: &syn::Ident,
    ns: &str,
    ig: &syn::ImplGenerics<'_>,
    tg: &syn::TypeGenerics<'_>,
    wc: Option<&syn::WhereClause>,
...
fn gen_tuple(
    ...
    let ra: Vec<_> = (0..n)
        .map(|i| {
            let idx = syn::Index::from(i);
            let s = i.to_string();
            quote! {#s => Some(&self.#idx as &dyn euca_reflect::Reflect)}
        })
        .collect();
    let ma: Vec<_> = (0..n)
        .map(|i| {
            let idx = syn::Index::from(i);
            let s = i.to_string();
            quote! {#s => Some(&mut self.#idx as &mut dyn euca_reflect::Reflect)}
        })
        .collect();
    ...
    let fi: Vec<_> = (0..n)
        .map(|i| {
            let idx = syn::Index::from(i);
            let s = i.to_string();
            quote! {euca_reflect::FieldInfo{name:#s,type_name:self.#idx.type_name()}}
        })
        .collect();
    let c = cm();
    quote! { impl #ig euca_reflect::Reflect for #name #tg #wc {
...
fn gen_enum(
    ...
    let arms: Vec<_> = data
        ...
        .map(|v| {
            ...
            let vs = vi.to_string();
            match &v.fields {
                Fields::Unit => quote! {#name::#vi => #vs},
                Fields::Unnamed(_) => quote! {#name::#vi(..) => #vs},
                Fields::Named(_) => quote! {#name::#vi{..} => #vs},
            }
        })
        .collect();
    let c = cm();
