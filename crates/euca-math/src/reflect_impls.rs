//! `Reflect` implementations for math types.
//!
//! Enabled by the `reflect` feature.
...
/// Implements Reflect for a struct with named f32 fields (x, y, z, optionally w).
        ...
        impl Reflect for $ty {
            ...
            fn type_info(&self) -> TypeInfo {
                TypeInfo {
                    name: $name,
                    fields: vec![
                        $( FieldInfo { name: stringify!($field), type_name: "f32" }, )+
                    ],
                }
            }
...
impl Reflect for Mat4 {
    ...
    fn type_info(&self) -> TypeInfo {
        TypeInfo {
            name: "Mat4",
            fields: vec![FieldInfo { name: "cols", type_name: "[[f32; 4]; 4]" }],
        }
    }
